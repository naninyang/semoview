...
const GPT = ({ fieldName, fieldValue, question }: { fieldName: string; fieldValue: string; question: string }) => {
  const [answer, setAnswer] = useState<string>('');
  const [idxList, setIdxList] = useState<string[]>([]);

  useEffect(() => {
    const fetchMovies = async () => {
      const response = await fetch(`/api/gpt?fieldName=${fieldName}&fieldValue=${fieldValue}&question=${question}`, {
        method: 'GET',
      });

      const data = await response.json();
      setAnswer(data.answer);
      setIdxList(data.idxList);
    };

    fetchMovies();
  }, [fieldName, fieldValue, question]);
...
function RecommendDetail({
  ...
  recommendData: RecommendParmalinkData;
  recommendId: string;
}) {
  ...
  return (
    <main className={`${footer.recommend} ${styles.recommend}`}>
      <Seo
        pageTitles={`${recommendData.attributes.subject} - ${originTitle}`}
        pageTitle={`${recommendData.attributes.subject}`}
        pageDescription={`${recommendData.attributes.description}`}
        pageImg={`https://cdn.dev1stud.io/semoview/ai/ai-${recommendId.substring(14)}.webp?ts=${timestamp}`}
      />
          ...
          <>
            <div className="headline">
              <h1 className="April16thPromise">
                <em>{recommendData.attributes.subject}</em>
              </h1>
            </div>
            <article>
              <div className={styles.chloe}>
                <BlockQuote data={recommendData.attributes.chloe} cite="Chloe" />
              </div>
              <div className={styles.gpt}>
                ...
                {recommendData.attributes.fieldName ? (
                  <GPT
                    fieldName={recommendData.attributes.fieldName}
                    fieldValue={recommendData.attributes.fieldValue}
                    question={recommendData.attributes.question}
                  />
                ) : (
                  <BlockQuote data={recommendData.attributes.gpt} cite="ChatGPT 4o" />
                )}
              </div>
            </article>
            {recommendData.attributes.fieldName ? (
              <p>The responses are generated by ChatGPT 3.5 Turbo</p>
            ) : (
              <p>Powered by ChatGPT 4o.</p>
            )}
          </>
        )}
      </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (recommendId && typeof recommendId === 'string') {
    ...
    const recommendResponse = (await ai.json()) as {
      data: RecommendParmalinkData;
    };

    recommendData =
      formatDateDetail(recommendResponse.data.attributes.createdAt) === recommendId.substring(0, 14) &&
